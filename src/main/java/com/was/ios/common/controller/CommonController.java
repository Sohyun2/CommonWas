...
public class CommonController {
	...
	public ResponseEntity<JSONObject> login(HttpServletRequest request) {		
		/*
		 * 1. db에서 login값 확인
		 * 2. login정보 확인되면 토큰 발급해서 클라이언트단으로 보내주기	
		*/
		...
		// 일치하는 login정보가 없음
		if(loginObject == null)	{
			return new ResponseEntity<JSONObject>(new ResponseObject().getResponse(Fail, null), HttpStatus.UNAUTHORIZED);
		}
		
		// login정보가 존재하면 token발급
		String token = getToken(loginObject);	
		
		return new ResponseEntity<JSONObject>(new ResponseObject().getResponse(Success , token), HttpStatus.OK);
	}
	
	@PostMapping(value="/test")
	public ResponseEntity<String> tokenTest(HttpServletRequest request) {
		String token = request.getParameter("token");
		
		boolean isExist = this.verifyToken(token);
		
		// 존재하지않으면 401에러 발생
		if(!isExist) {
			return new ResponseEntity<String>("잘못된 토큰", HttpStatus.UNAUTHORIZED);
		}		
